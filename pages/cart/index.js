...
export default function Cart() {
  ...
  async function increaseProduct(id) {
    addProduct(id);
        ...
        // Llamada al backend para aumentar la cantidad en la base de datos
        try {
          ...
          if (product) {
            ...
            const response = await axios.put('/api/products', { 
              ...
              topprod: product.topprod,
              brand: product.brand,
              sizes: product.sizes,
              cantidad: newCantidad 
            });
            if (response.data) {
              toast.success('Product quantity updated in database!');
            } else {
              toast.error('Error updating product quantity in database!');
            }
          }
        } catch (error) {
  ...
  async function decreaseProduct(id) {
    ...
    // Llamada al backend para disminuir la cantidad en la base de datos
    try {
      const product = products.find(p => p._id === id);
      if (product) {
        const newCantidad = product.cantidad - 1;
